...
<form class="content" action="pending_late_time_in.php" method="post">
	...
	<table class="table table-bordered table-striped" align="center">
		...
		if($_SESSION['type'] == 'HR'){
			...
			$sql = "SELECT employeeNum,fullname,id, request_type, reason, is_approved FROM form WHERE request_type='OFFICIAL BUSINESS' AND dept_approved = 'APPROVED' AND is_approved = 'PENDING'";
...
if(isset($_POST['update']))
{
   ...
   $mail = new PHPMailer\PHPMailer\PHPMailer();                              // Passing `true` enables exceptions
    ...
    // $name = $_POST['name'];
    //Recipients
    $mail->setFrom('arnazdj@gmail.com', 'Official Business Form Update');
    $mail->addAddress($email, 'Joe User');     // 

    //Content
    $mail->isHTML(true);                                  // Set email format to HTML
    $mail->Subject = 'Department Official Business Form Update';
    $mail->Body    = 'Sample1.';
    ...
    $mail->send();
			...
			$sql = "SELECT employeeNum,fullname,id, request_type, reason, is_approved, department FROM form WHERE is_approved='PENDING' AND request_type='OFFICIAL BUSINESS' AND department = '$test'";
			$result1 = $conn-> query($sql);
