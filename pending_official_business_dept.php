...
<form class="content" action="pending_official_business_dept.php" method="post">
	 <div class="formtable">
	<table class="table table-bordered table-striped" align="center">
		<tr>
			...
			<th style="border: 1px solid #bb9121; color: #bb9121;">Situation(No-In/No-Out/Both)</th>
			<th style="border: 1px solid #bb9121; color: #bb9121;">Employee`s Reason</th>
			<th style="border: 1px solid #bb9121; color: #bb9121;">No-In</th>
			<th style="border: 1px solid #bb9121; color: #bb9121;">No-Out</th>
		...
		if($_SESSION['type'] == 'HR'){
			...
			$sql = "SELECT * FROM form WHERE request_type='OFFICIAL BUSINESS' AND dept_approved = 'APPROVED' AND is_approved = 'PENDING'";
   ...
   $hrDateUpdate =$_POST['hrDateUpdate'];
      foreach ($_POST["id"] as $id ) {
        ...
        $result = mysqli_query($conn, $query);
            if($approve == 'APPROVED'){
                    ...
                    $mail->setFrom('localhost.roycehotel@gmail.com', 'Official Business Form Approved');
                    $mail->addAddress($email);     // 

    //Content
                    $mail->isHTML(true);                                  // Set email format to HTML
                    $mail->Subject = 'Official Business Form Update HR';
            ...
            if($approve == 'REJECT'){
                    ...
                    $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
                    $mail->Port = 587;                                    // TCP port 
                    $mail->setFrom('localhost.roycehotel@gmail.com', 'Official Business Form Rejected');
                    $mail->addAddress($email);     // 
    ...
    //Content
          ...
          echo '</td><td style="border: 1px solid #bb9121; color: #bb9121;"><br>'.$row["employeeNum"].'';
            ...
            echo '</td><td style="border: 1px solid #bb9121; color: #bb9121;"><br>'.$row["request_type"].'';

            echo  '<td style="border: 1px solid #bb9121; color: #bb9121;"><br>
            '.$row["situation"].'</td>';

            echo '</td><td style="border: 1px solid #bb9121; color: #bb9121;"><br><textarea name = reason['.$row["id"].'] type = text readonly = readonly >'. $row["reason"].'</textarea>';

            echo  '<td style="border: 1px solid #bb9121; color: #bb9121;"><br>
            '. date("g:i a", strtotime($row["no_in"])).'</td>';

            echo  '<td style="border: 1px solid #bb9121; color: #bb9121;"><br>
            '. date("g:i a", strtotime($row["no_out"])).'</td>';

            echo '</td><td style="border: 1px solid #bb9121; color: #bb9121;"><br>
            <select class="purpose" style="border: 1px solid #bb9121; color: #bb9121;background-color:#fffacd;" id="purpose"  name = is_approved['.$row["id"].']><option value = "PENDING">PENDING</option><option value = "APPROVED">APPROVED</option><option value = "REJECT">REJECT</OPTION></select></td>';
            ...
            echo  '</td><td id="business2" style="display:none;"><br><textarea style="width:150px;" placeholder="Optional..." name = reasonHr['.$row["id"].'] type = text></textarea></td></tr>';
				}
				echo "</table>";
			}
			else{
			...
			$sql = "SELECT * FROM form WHERE dept_approved='PENDING' AND request_type='OFFICIAL BUSINESS' AND department = '$test'";
			$result1 = $conn-> query($sql);
...
echo '<tr><input type = hidden  name = id['.$row["id"].'] type = text readonly = readonly value ="'.$row["id"].'"></input>';
          ...
          echo '</td><td style="border: 1px solid #bb9121; color: #bb9121;"><br>'.$row["employeeNum"].'';
            ...
            echo '</td><td style="border: 1px solid #bb9121; color: #bb9121;"><br>
            <select class="purpose"  style="border: 1px solid #bb9121; color: #bb9121;background-color:#fffacd;" id="purpose"  name = dept_approved['.$row["id"].']><option value = "PENDING">PENDING</option><option value = "APPROVED">APPROVED</option><option value = "REJECT">REJECT</OPTION></select></td>';

            echo '<input style="text-align:center;border: 1px solid #bb9121; color: #bb9121;background-color:#fffacd;" hidden required name="hrDateUpdate" type="date" value='.date("Y-m-d").'></input>';

            echo  '</td><td id="business2" style="display:none;"><br><textarea style="width:150px;" placeholder="Optional..." name = reasonDept['.$row["id"].'] type = text></textarea></td></tr>';
				}
				echo "</table>";
			}
			else{
