...
<form class="content" action="pending_no_time_in_dept.php" method="post">
   <div class="formtable">
  <table  class="table table-bordered table-striped" align="center">
    ...
    if($_SESSION['type'] == 'HR'){
      ...
      $sql = "SELECT * FROM form WHERE dept_approved='APPROVED' AND request_type='NO-IN' AND is_approved = 'PENDING'";
...
if(isset($_POST['update']))
{
   ...
   $hrDateUpdate =$_POST['hrDateUpdate'];
      foreach ($_POST["id"] as $id ) {
        ...
        $result = mysqli_query($conn, $query);
            if($approve == 'APPROVED'){
                    ...
                    $mail->setFrom('localhost.roycehotel@gmail.com', 'No-In Form Approved');
                    $mail->addAddress($email);     // 

    //Content
                    $mail->isHTML(true);                                  // Set email format to HTML
                    $mail->Subject = 'No-InForm Update HR';
    ...
    //Content
                    $mail->isHTML(true);                                  // Set email format to HTML
                    $mail->Subject = 'No-In Form Update HR';
...
$query = "UPDATE `form` SET `is_approved`='".$approve."',`reasonDept`='".$reasonDept."',`deptDateUpdate`='".$deptDateUpdate."'  WHERE `id` = $id LIMIT 1";  
   ...
   $result = mysqli_query($conn, $query);
      ...
      if($approve == 'APPROVED'){
                    ...
                    $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
                    $mail->Port = 587;                                    // TCP port 
                    $mail->setFrom('localhost.roycehotel@gmail.com', 'No-InForm Approved Department Head');
                    $mail->addAddress($email);     // 

    //Content
                    $mail->isHTML(true);                                  // Set email format to HTML
                    $mail->Subject = 'No-In Form Update Department Head';
            ...
            if($approve == 'REJECT'){
                    ...
                    $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
                    $mail->Port = 587;                                    // TCP port 
                    $mail->setFrom('localhost.roycehotel@gmail.com', 'No-In Form Rejected');
                    $mail->addAddress($email);     // 
