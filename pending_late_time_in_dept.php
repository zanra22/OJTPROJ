...
<form class="content" action="pending_late_time_in.php" method="post">
   <div class="formtable">
	<table class="table table-bordered table-striped" align="center">
        ...
        $sql = "SELECT employeeNum,fullname,id, request_type, reason, is_approved FROM form WHERE request_type='LATE TIME-IN' AND dept_approved = 'APPROVED' AND is_approved = 'PENDING'";			
...
if(isset($_POST['update']))
{
   ...
   $approved = mysqli_real_escape_string($conn,$_POST["dept_approved"][$id]);


   $query = "UPDATE `form` SET `fullname`='".$fname."',`reason`='".$lname."',`is_approved`='".$approve."', `request_type`='".$mname."',`dept_approved`='".$approved."' WHERE `id` = $id LIMIT 1";
   ...
   $mail = new PHPMailer\PHPMailer\PHPMailer();                              // Passing `true` enables exceptions
    ...
    // $name = $_POST['name'];
    //Recipients
    $mail->setFrom('arnazdj@gmail.com', 'Late Time In Form Update');
    $mail->addAddress($email, 'Joe User');     // 

    //Content
    $mail->isHTML(true);                                  // Set email format to HTML
    $mail->Subject = 'HR Late Time In Form Update';
    $mail->Body    = 'Sample1.';
    ...
    $mail->send();
       ...
       if ($result1-> num_rows > 0) {
        ...
        echo "0 result";
			...
			$sql = "SELECT employeeNum,fullname,id, request_type, reason, is_approved, department FROM form WHERE is_approved='PENDING' AND request_type='LATE TIME-IN' AND department = '$test'";
			$result1 = $conn-> query($sql);
...
if(isset($_POST['update']))
{
   ...
   $mail = new PHPMailer\PHPMailer\PHPMailer();                              // Passing `true` enables exceptions
    ...
    //Content
    $mail->isHTML(true);                                  // Set email format to HTML
    $mail->Subject = 'Department Late Time In Form Update';
    $mail->Body    = 'Sample1.';
    // $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
