...
function differenceInMinutes($startOfShift,$undertime){
  $starttimestamp = strtotime($startOfShift);
  $endtimestamp = strtotime($undertime);
  $difference = abs($endtimestamp - $starttimestamp)/3600;
  return $difference;
}
if(!empty($_POST["submit"])) {
  $hours_difference = differenceInMinutes($_POST["startOfShift"],$_POST["undertime"]); 
...
<form id="formID" action="undertime.php" method="post">
			...
			if (isset($_SESSION['employee_N'])):
              ...
              <?php
			    		...
			    		<td style="border: 1px solid #bb9121; color: #bb9121;"><input style="text-align:center;border: 1px solid #bb9121; color: #bb9121;background-color:#fffacd;" required name="planDate" type="date" value="<?php if(!empty($_POST["planDate"])) { echo $_POST["planDate"]; } ?>"></input></td>
      					...
      					<td style="border: 1px solid #bb9121; color: #bb9121;"><input style="text-align:center;border: 1px solid #bb9121; color: #bb9121;background-color:#fffacd;" required name="undertime" value="<?php if(!empty($_POST["undertime"])) { echo $_POST["undertime"]; } ?>" type="time" placeholder="Start of Overtime"></td>
...
<button class="btn btn-outline-warning btn-lg" align="center" type="submit" name="undertime_reg" value="Submit">Submit</button>
